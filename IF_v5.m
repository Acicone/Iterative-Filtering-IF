function [IMF,logM] = IF_v5(f,options,M)


%
%  function IMF = IF_v5(f,options)
%
...
if nargin < 1,  help IF_v5; return; end
...
while countIMFs < options.IF.NIMFs && k>=options.IF.ExtPoints && toc<options.maxTime
    ...
    if isempty(M) || length(M)<countIMFs
        if isa(options.IF.alpha,'char')
            if strcmp(options.IF.alpha,'ave') % Using an average mask length
                m = 2*round(N/k*options.IF.Xi);
            elseif strcmp(options.IF.alpha,'Almost_min') % Using an almost min mask length
                if 2*round(options.IF.Xi*(ceil((max(diffMaxmins)-min(diffMaxmins))/10)+min(diffMaxmins)))<2*round(N/k*options.IF.Xi)
                    m = 2*round(options.IF.Xi*(ceil((max(diffMaxmins)-min(diffMaxmins))/10)+min(diffMaxmins)));
                else
                    m = 2*round(N/k*options.IF.Xi);
                end
                if countIMFs>1
            ...
            m = 2*round(options.IF.Xi*(max(diffMaxmins)*options.IF.alpha+min(diffMaxmins)*(1-options.IF.alpha)));
        end
    else
        m=M(countIMFs);
    end
...
%         set(figMask,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_IF_v5.mat'],'IMF','f','logM','-v7.3');
    end
end %end of while
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_IF_v5.mat'],'IMF','logM','-v7.3');
end
