function [IMF,logM] = IF_v8_2(f,options,M)


%
%  function IMF = IF_v8_2(f,options)
%
...
%
%   See also SETTINGS_IF_V1, GETMASK_V1, MAXMINS_v3_2, PLOT_IMF_V8.
%
...
if nargin < 1,  help IF_v8_2; return; end
...
f_pp=f;
f_pp(abs(f)<=10^-18)=[];
maxmins_pp=Maxmins_v3_2(f_pp,options.IF.extensionType);
diffMaxmins_pp=diff(maxmins_pp);
N_pp=length(f_pp);
k_pp = length(maxmins_pp);

if options.plots>=1
    maxmins=Maxmins_v3_2(f,options.IF.extensionType);
    figMask=figure;
...
while countIMFs < options.IF.NIMFs && k_pp>=options.IF.ExtPoints && toc<options.maxTime
    ...
    while SD>options.IF.delta && inStepN < options.IF.MaxInner
        ...
        if options.verbose>0 || options.plots>=1
            [Maxs,Mins]=Maxmins_v3_2(h,options.IF.extensionType);
            if not(isempty(Maxs))
...
%         set(figMask,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_IF_v8_2.mat'],'IMF','f','logM','-v7.3');
    end
end %end of while
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_IF_v8_2.mat'],'IMF','logM','-v7.3');
end
...
function varargout = Maxmins_v3_2(f,extensionType)
% Based on version 3
