function [IMF,logM] = IF_v6_2(f,options,M)
...
%
%   See also SETTINGS_IF, GET_MASK_V1, MAXMINS_v3_1, PLOT_IMF_V8.
%
...
f_pp=f;
f_pp(abs(f)<=10^-18)=[];
maxmins_pp=Maxmins_v3_1(f_pp,options.IF.extensionType);
diffMaxmins_pp=diff(maxmins_pp);
N_pp=length(f_pp);
k_pp = length(maxmins_pp);

if options.plots>=1
    maxmins=Maxmins_v3_1(f,options.IF.extensionType);
    figMask=figure;
...
while countIMFs < options.IF.NIMFs && k_pp>=options.IF.ExtPoints && toc<options.maxTime
    ...
    while SD>options.IF.delta && inStepN < options.IF.MaxInner
        ...
        if options.verbose>0 || options.plots>=1
            [Maxs,Mins]=Maxmins_v3_1(h,options.IF.extensionType);
            if not(isempty(Maxs))
...
function varargout = Maxmins_v3_1(f,extensionType)
% Basedon version 3
% Minor revision: added for constant extention the checking for Mins and
% Maxs emptiness
